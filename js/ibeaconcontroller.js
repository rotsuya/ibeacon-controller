    ...
    for (var i = 0, il = data.length; i < il; i++) {
        ...
        for (var j = 0, jl = person.data.length; j < jl; j++) {
            ...
            if (isNewBeacon) {
                ...
                //beacon を map に表示する
                iBeaconMap.addBeacon(beacon.uuid, beacon.major, beacon.minor);
                return;
            }
        }
        ...
        if (this.persons.indexOf(person.id) === -1) {
            ...
            //person を map から消す
            iBeaconMap.addPerson(person.id, person.x, person.y);
            return;
        }

        //既に表示済みの person.id の場合は、移動する
        iBeaconMap.movePerson(person.id, person.x, person.y);
    }
};
